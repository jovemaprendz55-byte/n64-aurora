...
> A Nova Arquitetura do React Native permanece desativada neste release. O SDK 54 ainda permite essa configuração e ela é necessária enquanto o projeto usa o NDK `26.1.10909125` do Mupen64Plus-AE: a compilação Fabric dessa combinação tenta usar `std::format`, que não está disponível nesse NDK. Uma migração futura para a Nova Arquitetura exigirá testar uma atualização coordenada do NDK e dos módulos nativos.
